        ...
        <div class="shop-card">
            ...
            <div class="shop-card__about-box">
                ...
                <div class="shop-card__about-bottom">
                    <a class="shop-card__about-detail" href="{{ route('shop.show', ['shop' => $shop->id]) }}">詳しくみる</a>
                    @if(in_array($shop->id, $favorite_shop_ids))
                        <form class="favorite-form" action="{{ route('favorite.destroy') }}" method="post">
                            @csrf
                            @method('delete')
                            <input type="hidden" name="shop_id" value="{{ $shop->id }}">
                            <button class="shop-card__about-favorite">
                    ...
                    @else
                        <form class="favorite-form" action="{{ route('favorite.store') }}" method="post">
                            @csrf
                            <input type="hidden" name="shop_id" value="{{ $shop->id }}">
                            <button class="shop-card__about-favorite">
